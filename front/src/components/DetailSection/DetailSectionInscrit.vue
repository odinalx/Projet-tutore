...
const section = ref({
  ...
  seances: [
    ...
    { jour: 'vendredi', heure: '19:00' },
  ],
  telephone: '+33 6 12 34 56 78',
  email: 'johanne.football@me.com',
  responsable: 'Johanne S.',
  lieu: 'Salle 101',
})
</script>
...
<template>
  <div class="container mx-auto p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <tbody>
              <tr>
                <td class="border border-gray-200 px-4 py-2">
                  {{ section.seances.find((s) => s.jour === 'lundi')?.heure || '' }}
                </td>
                <td class="border border-gray-200 px-4 py-2">
                  {{ section.seances.find((s) => s.jour === 'mardi')?.heure || '' }}
                </td>
                <td class="border border-gray-200 px-4 py-2" />
                <td class="border border-gray-200 px-4 py-2" />
                <td class="border border-gray-200 px-4 py-2">
                  {{ section.seances.find((s) => s.jour === 'vendredi')?.heure || '' }}
                </td>
                <td class="border border-gray-200 px-4 py-2" />
                <td class="border border-gray-200 px-4 py-2" />
              </tr>
      ...
      <Card>
        <CardHeader>
          <h2 class="text-lg font-semibold">Mes Informations</h2>
        </CardHeader>
        <CardContent>
          <ul class="space-y-2">
            <li><strong>État:</strong> {{ section.etat }}</li>
            <li><strong>Lieu:</strong> {{ section.lieu }}</li>
          </ul>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <ul class="space-y-2">
            <li><strong>Responsable:</strong> {{ section.responsable }}</li>
            <li>
              <strong>Email:</strong>
              <a :href="`mailto:${section.email}`" class="text-blue-500 hover:underline">{{
                section.email
              }}</a>
            </li>
            <li>
              <strong>Numéro:</strong>
              <a :href="`tel:${section.telephone}`" class="text-blue-500 hover:underline">{{
                section.telephone
              }}</a>
            </li>
          </ul>
